...
### Role Selection Gate (Mandatory)

Before any implementation begins, the agent must explicitly identify:

* the primary role responsible for the task
* the supporting roles, if any
* the affected domain (backend, frontend, database, QA, security, review)
* whether the task requires clarification before execution

If the role is unclear, the task must pause and the appropriate role must be selected before proceeding.
