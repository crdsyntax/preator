...
* every task must begin by identifying the correct agent role or execution model
* the selected role must match the task domain and scope
* if the domain is ambiguous, the agent must pause and request clarification before acting
* implementation must never start under an undefined or generic role
